...
class PrepareRawData(object):
    ...
    def get_raw(self, file_name: str = '', raw_id: int = np.nan):
        """
        Get raw data, if raw_id is sent then it will filter the data
            ...
            try:
                ...
                if not pd.isnull(raw_id):
                    ...
                    try:
                        ...
                        self.df = self.df[self.df.Id==raw_id]
                    except:
    ...
    def process_categoric(self, raw_id: bool = np.nan):
        """
        ...
        self.df[self.cat_cols] = self.df[self.cat_cols].fillna('other')
        ...
        #save dummies
        self.df_cat = pd.DataFrame(enc.transform(self.df[self.cat_cols]).toarray(),
            columns = enc.get_feature_names(self.cat_cols)
            )


    def impute_numerical(self, impute_strategy: str = 'constant', raw_id: bool = np.nan):
        """
        ...
        self.df = pd.DataFrame(imputer.transform(self.df[self.numeric_cols]), 
            columns = self.numeric_cols
            )


    def split_rnd_set(self, split_size: float = .3):
        """
        ...
        df_train.to_csv('data/df_processed_train.csv', index=False)
        df_test.to_csv('data/df_processed_test.csv', index=False)
        ...
        print('\n     Split done')
        print('          data/df_processed_train.csv shape:', df_train.shape)
        print('          data/df_processed_test.csv shape:', df_test.shape)
    ...
    def get_single_obs(self):
        ...
        del self.df_y['index']
        return pd.concat([self.df, self.df_cat, self.df_y], axis=1)
